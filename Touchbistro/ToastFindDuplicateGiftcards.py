...
def processMenu(csvFile):
    with open(csvFile) as f:
        ...
        for line in reader:
            ...
            if Imported_Number:
                if Imported_Number not in cardDict:
                    cardDict[Imported_Number] = Card_Number
                else:
                    idDict[Card_Number] = Imported_Number
            
    with open('dupcards.csv', 'w', newline='') as fout:
        writer = csv.writer(fout)
        for id in idDict:
            #print(f'{Cnumber},{cardDict[Cnumber]}')
            print (id,',',idDict[id])
            writer.writerow ([id,idDict[id]])
